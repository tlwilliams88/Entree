...
using Entree.Core.Extensions.ShoppingCart;

using Entree.Core.Helpers;
...
//cart.SubTotal += (decimal)PricingHelper.GetPrice(qty, item.CasePriceNumeric, item.PackagePriceNumeric, item.Each, item.CatchWeight, item.AverageWeight, pack);
		...
		public void LookupProductDetails(UserProfile user, UserSelectedContext catalogInfo, ShoppingCart cart, List<Entree.Core.Models.Lists.ListItemModel> notes = null)
		{
			if (cart.Items == null)
				return;
			...
			var notesHash = new Dictionary<string, Entree.Core.Models.Lists.ListItemModel>();
			if (notes != null)
