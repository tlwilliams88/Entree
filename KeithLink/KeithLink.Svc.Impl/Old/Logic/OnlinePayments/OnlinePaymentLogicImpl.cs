...
using Entree.Core;
using Entree.Core.Enumerations;
using Entree.Core.Extensions;
using Entree.Core.Extensions.Messaging;
using Entree.Core.Extensions.OnlinePayments;
using Entree.Core.Extensions.OnlinePayments.Customer;
...
using EFCustomer = Entree.Core.Models.OnlinePayments.Customer.EF;
using EFInvoice = Entree.Core.Models.OnlinePayments.Invoice.EF;
    ...
    public class OnlinePaymentLogicImpl : IOnlinePaymentsLogic
    {
        ...
        private void GetInvoiceTransactions(InvoiceModel invoice)
        {
            ...
            if (transactions != null) {
                invoice.InvoiceAmount = transactions.Where(x => x.InvoiceType == Entree.Core.Constants.INVOICETRANSACTIONTYPE_INITIALINVOICE)
                                                    .Select(x => x.AmountDue)
                                                    .FirstOrDefault();

                if (transactions.Where(x => x.InvoiceType == Entree.Core.Constants.INVOICETRANSACTIONTYPE_CREDITMEMO)
                               .Count() > 0)
                {
