...
namespace KeithLink.Svc.Impl.Tests.Integration.Repository.Lists {
    public class RecentlyOrderedHeaderRepositoryTests {
        private static IRecentlyOrderedListHeadersRepository MakeRepo() {
            IContainer diMap = DependencyMapFactory.GetTestsContainer()
                                                   .Build();
            return diMap.Resolve<IRecentlyOrderedListHeadersRepository>();
        }
        ...
        public class GetRecentlyOrderedItemsHeader : MigratedDatabaseTest {
            [Fact]
            public void GoodCustomer_ReturnsExpectedHeader() {
                ...
                // act
                var result = repo.GetRecentlyOrderedHeader(Guid.Parse(GOOD_USER_ID), fakeCustomer);
        ...
        public class SaveRecentlyOrderedListHeader : MigratedDatabaseTest {
            ...
            public void GoodHeader_SavesCorrectBranchId() {
                ...
                // act
                repo.Save(header);
                var results = repo.GetRecentlyOrderedHeader(Guid.Parse(NEW_USER_ID), customer);
