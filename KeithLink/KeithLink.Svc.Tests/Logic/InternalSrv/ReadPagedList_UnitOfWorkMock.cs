...
{
    public class ReadPagedList_UnitOfWorkMock : IUnitOfWork
    {
		...
		public BEKDBContext Context
		{
			get
			{
				if (_context == null)
				{
                    ...
                    var mockItemHistory = new FakeDbSet<ItemHistory>();
					#endregion
                    ...
                    IQueryable<ItemHistory> historyData = Builder<ItemHistory>.CreateListOfSize(5).Build().AsQueryable();
                    mockItemHistory.SetData(historyData);
                    ...
                    mockContext.Setup(c => c.Set<ItemHistory>()).Returns(mockItemHistory.Object);
