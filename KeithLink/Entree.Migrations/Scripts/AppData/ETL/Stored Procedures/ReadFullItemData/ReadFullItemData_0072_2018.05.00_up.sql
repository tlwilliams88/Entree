...
BEGIN
	...
	SELECT 
        ...
        ,i.PriceListCode AS PriceListCode
		,[ETL].initcap(pricelistcodes.CategoryName) as PriceListCodeCategoryName
		,SUBSTRING(i.PriceListCode, 1,2) + '000' AS ParentPriceListCode
	FROM  
		ETL.Staging_ItemData i 
		inner join ETL.Staging_Departments c on i.CategoryId = c.DepartmentId
		left outer join ETL.Staging_Category pricelistcodes on pricelistcodes.CategoryId = i.PriceListCode
