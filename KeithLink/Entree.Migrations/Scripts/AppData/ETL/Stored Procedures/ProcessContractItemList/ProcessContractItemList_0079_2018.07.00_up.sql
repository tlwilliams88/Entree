    ...
    INSERT -- insert shadow item into delta list
    	...
    	WHERE 
	    	NOT EXISTS (
		    	SELECT
			    	'x'
				    FROM [ETL].[Staging_BidContractDetail] bcdExists
    				INNER JOIN 
	    				[ETL].[Staging_CustomerBid] cbExists
		    			ON cbExists.BidNumber=bcdExists.BidNumber AND cbExists.DivisionNumber = bcdExists.DivisionNumber
			    	INNER JOIN 
				    	List.[ContractHeaders] lExists
					    ON lExists.CustomerNumber = ltrim(rtrim(cbExists.CustomerNumber)) and lExists.BranchId = ltrim(rtrim(cbExists.DivisionNumber))
    				WHERE
	    				ltrim(rtrim(ItemNumber)) = li.ItemNumber
		    			AND CASE WHEN bcdExists.ForceEachOrCaseOnly = 'B' THEN 1 ELSE 0 END = li.Each
			    		AND ltrim(rtrim(cbExists.CustomerNumber)) = l.CustomerNumber 
				    	and ltrim(rtrim(cbExists.DivisionNumber)) = l.BranchId
	    		)
    		AND NOT EXISTS (
		    	SELECT
			    	'x'
    				...
    				WHERE
				    	...
				    	AND lid.ParentList_Id = li.HeaderId
    			)

	UPDATE -- update customer contract lists name from staging customerbids from current customer bids 
		l
        SET l.ContractId = LTRIM(RTRIM(cb.BidNumber))
        FROM 
            [ETL].[Staging_CustomerBid] as cb
            INNER JOIN [List].[ContractHeaders] l ON l.[CustomerNumber] = LTRIM(RTRIM(cb.[CustomerNumber])) 
				                                       AND l.[BranchId] = LTRIM(RTRIM(cb.[DivisionNumber])) 
